...
// JG Jul 2012 Added Docx Contributor
	...
	void btnSaveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSaveActionPerformed
            // Add your handling code here:
            RemovableStorageHandler usbHandler = RemovableStorageHandler.getInstance();
...
//            }
            fileChooser.setFileFilter(new JRPdfSaveContributor(getLocale(), resourceBundle));

            String date = LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd-MM-yyyy_HH.mm"));
            String file_name = JPrincipalApp.getTitle_name().replace(" ", "-") + "-" + date;
            ...
            } else if (usbHandler.getLatestUSBDevice() != null) {
                fileChooser.setCurrentDirectory(usbHandler.getLatestUSBDevice());
    ...
    /**
     *
     * @return @return @throws net.sf.jasperreports.engine.JRException
     * @throws JRException
     */
